...
public class PropertyValueAPI {
    ...
    private Map<String, Object> getVersionModuleSeverity() {
        Map<String, Object> modelMap = new HashMap<String, Object>();
        List<PropertyValue> list = new ArrayList<PropertyValue>();
        list = propertyValueService.getMainVersion();   //版本号
        modelMap.put("realMainVersion", list);
        list = propertyValueService.getAllVersion();    //所有主版本
        modelMap.put("allMainVersion", list);
        list = propertyValueService.getAllModule();     //模块
        modelMap.put("getAllModule", list);
        list = propertyValueService.getHeavySeverity(); //严重度
