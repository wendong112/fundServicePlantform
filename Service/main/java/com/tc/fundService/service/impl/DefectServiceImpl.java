...
import com.tc.fundService.dao.DefectDao;
...
public class DefectServiceImpl implements DefectService{
    ...
    public Defect getDefectById(int id) {
        ...
        return defectDao.queryDefectById(id);
    }


    @Override
    public List<Defect>  getDefectBySeverityId(int severityId) {
        return defectDao.queryDefectBySeverityId(severityId);
    }
    ...
    public boolean addDefect(Defect defect) {
            if(defect.getTitle() != null && !"".equals(defect.getTitle())){
                ...
                try {
                    int effectedNum = defectDao.insertDefect(defect);
                    if(effectedNum > 0) {
                        return true;
                    }else {
    ...
    public boolean modifyDefect(Defect defect) {
        ...
        if (defect.getId() != null && defect.getId() > 0) {
            ...
            try {
                // 更新区域信息
                int effectedNum = defectDao.updateDefect(defect);
                if (effectedNum > 0) {
                    return true;
                } else {
    ...
    public boolean deleteDefect(int id) {
        {
            if (id > 0) {
                try {
                    // 删除缺陷信息
                    int effectedNum = defectDao.deleteDefect(id);
                    if (effectedNum > 0) {
                        return true;
                    } else {
